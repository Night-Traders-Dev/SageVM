...
test:
	...
	@if [ ! -f .deps/SageLang/core/sage ]; then \
		$(MAKE) -C .deps/SageLang/core SAGE_NO_GUI=1 SAGE_NO_NET=1 SAGE_NO_GPU=1 CFLAGS_EXTRA="-DSAGE_NO_GUI -DSAGE_NO_NET -DSAGE_NO_GPU" LDFLAGS="-lm -lpthread -ldl" -j$$(nproc); \
	fi
	@$(MAKE) all
