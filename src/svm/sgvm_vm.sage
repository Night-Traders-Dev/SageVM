    ...
    proc run(self, code):
        ...
        while not halted and ip < code_len:
            ...
            if op == OP_GET_LOCAL:
                ...
                # Performance: C-level native truthiness evaluation bypassing multi-comparison OR chain
                if not cond: ip = target
                ...
                let cond = stack[stack_len-1]
                # Performance: C-level native truthiness evaluation bypassing multi-comparison OR chain
                stack[stack_len-1] = not cond
            elif op == OP_TRUTHY:
                let cond = stack[stack_len-1]
                # Performance: C-level native truthiness evaluation bypassing multi-comparison OR chain
                stack[stack_len-1] = not (not cond)
            elif op == OP_PRINT:
