...
- [ ] **Generator Opcodes**: Implement interpreter support for `OP_YIELD`, `OP_CREATE_GENERATOR`, and `OP_GENERATOR_NEXT` in `src/svm/sgvm_vm.sage`.
   - > ⚠️ **Encoding Mismatch**: Authoritative indices are 61, 62, and 63, but SageVM currently uses indices 90, 91, and 92.
- [ ] **Compiler Conformance**: Overhaul `src/svm/sgvm_compiler.sage` remapping logic to align with authoritative `bytecode.h` indices.
   - [ ] Local variable & Generator remapping (Update 88-92 to 59-63)
