...
- `OP_SET_LOCAL` (89): Set a local variable value (Authoritative index is 60; Collides with authoritative `BC_OP_GPU_UPDATE_UNIFORM`).
- `OP_YIELD` (90): Yield value from generator (Authoritative index is 61; Collides with authoritative `BC_OP_GPU_CMD_PUSH_CONST`).
- `OP_CREATE_GENERATOR` (91): Create generator object (Authoritative index is 62; Collides with authoritative `BC_OP_GPU_CMD_DISPATCH`).
...
- **Opcode Alignment Regression**: As of the latest sync (2026-07-12), the authoritative `bytecode.h` has introduced `BC_OP_GET_LOCAL` (59), `BC_OP_SET_LOCAL` (60), and generator opcodes (`BC_OP_YIELD`, `BC_OP_CREATE_GENERATOR`, `BC_OP_GENERATOR_NEXT`) at indices 61-63, which has shifted the entire GPU instruction block (indices 64-91). SageVM currently maintains a legacy mapping where GPU opcodes start at 59, leading to a 5-opcode shift across the entire Phase 16 instruction set and collisions for trailing SageVM-specific opcodes.
