...
- `OP_SET_LOCAL` (89): Set a local variable value (Collides with authoritative `BC_OP_GPU_UPDATE_UNIFORM`).
- `OP_YIELD` (90): Yield from generator (Collides with authoritative `BC_OP_GPU_CMD_PUSH_CONST`).
- `OP_CREATE_GENERATOR` (91): Define generator (Collides with authoritative `BC_OP_GPU_CMD_DISPATCH`).
- `OP_GENERATOR_NEXT` (92): Resume generator execution.
- `OP_HALT` (255): Unconditional VM termination.

### 9.2 Known Incompatibilities
- **Opcode Alignment Regression**: As of the latest sync (2026-07-10), the authoritative `bytecode.h` has introduced five new opcodes (`BC_OP_GET_LOCAL`, `BC_OP_SET_LOCAL`, `BC_OP_YIELD`, `BC_OP_CREATE_GENERATOR`, `BC_OP_GENERATOR_NEXT`) at indices 59-63, shifting the entire GPU instruction block (indices 64-91). SageVM currently maintains a legacy mapping where GPU opcodes start at 59, leading to a 5-opcode shift across the entire Phase 16 instruction set and critical collisions for trailing SageVM-specific opcodes.
