...
- **Opcode Alignment Regression**: As of the latest sync (2026-07-12), the authoritative `bytecode.h` has introduced a 5-opcode prefix (59-63) for locals and generators, which has shifted the entire GPU instruction block (indices 64-91). SageVM currently maintains a legacy mapping where GPU opcodes start at 59, leading to a 5-opcode shift across the entire Phase 16 instruction set and collisions for trailing SageVM-specific opcodes.
