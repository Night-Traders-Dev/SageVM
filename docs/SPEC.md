...
- `OP_GET_LOCAL` (88): Get local variable (Collides with authoritative `BC_OP_GPU_RESET_FENCE`).
- `OP_SET_LOCAL` (89): Set local variable (Collides with authoritative `BC_OP_GPU_UPDATE_UNIFORM`).
- `OP_YIELD` (90): Generator yield (Collides with authoritative `BC_OP_GPU_CMD_PUSH_CONST`).
- `OP_CREATE_GENERATOR` (91): Create generator (Collides with authoritative `BC_OP_GPU_CMD_DISPATCH`).
- `OP_GENERATOR_NEXT` (92): Advance generator.
- `OP_HALT` (255): Unconditional VM termination.

### 9.2 Known Incompatibilities
- **Opcode Alignment Regression**: As of the latest sync (2026-07-10), the authoritative `bytecode.h` has introduced local variable and generator opcodes (indices 59-63), which has shifted the entire GPU instruction block (indices 64-91). SageVM currently maintains a legacy mapping where GPU opcodes start at 59, leading to a 5-opcode shift across the entire Phase 16 instruction set and critical collisions for trailing SageVM-specific opcodes.
