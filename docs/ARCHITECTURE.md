...
> ⚠️ **Opcode Alignment Regression**: As of the latest sync, an expanded encoding mismatch has been detected. The authoritative `bytecode.h` has introduced `BC_OP_GET_LOCAL` (59), `BC_OP_SET_LOCAL` (60), and generator opcodes (61-63), shifting the entire GPU instruction block (formerly 59-86) to 64-91. SageVM currently maintains a legacy mapping where GPU opcodes start at 59, resulting in a **5-opcode shift** and multiple collisions for SageVM extensions (e.g., `OP_GET_LOCAL` at 88 vs authoritative 59).
...
> ⚠️ **Compiler Implementation Gap**: `sgvmc` (`src/svm/sgvm_compiler.sage`) currently lacks binary emission logic for `OP_MATH_PRINTM` (87). Local variable opcodes (88-89) and generator opcodes (90-92) are now correctly remapped in the compiler's second pass, but they diverge significantly from the new authoritative indices.
...
| OP_GPU_GET_TIME | 61 | 66 | gpu.get_time() [Collision: YIELD] |
| OP_GPU_KEY_PRESSED | 62 | 67 | gpu.key_pressed(key) [Collision: CREATE_GEN] |
| OP_GPU_KEY_DOWN | 63 | 68 | gpu.key_down(key) [Collision: GEN_NEXT] |
| OP_GPU_MOUSE_POS | 64 | 69 | gpu.mouse_pos() |
| OP_GPU_MOUSE_DELTA | 65 | 70 | gpu.mouse_delta() |
...
| OP_GPU_CMD_BEGIN_RP | 69 | 74 | gpu.cmd_begin_render_pass(...) |
| OP_GPU_CMD_END_RP | 70 | 75 | gpu.cmd_end_render_pass(cmd) |
| OP_GPU_CMD_DRAW | 71 | 76 | gpu.cmd_draw(...) |
| OP_GPU_CMD_BIND_GP | 72 | 77 | gpu.cmd_bind_graphics_pipeline(...) |
| OP_GPU_CMD_BIND_DS | 73 | 78 | gpu.cmd_bind_descriptor_set(...) |
| OP_GPU_CMD_SET_VP | 74 | 79 | gpu.cmd_set_viewport(...) |
| OP_GPU_CMD_SET_SC | 75 | 80 | gpu.cmd_set_scissor(...) |
| OP_GPU_CMD_BIND_VB | 76 | 81 | gpu.cmd_bind_vertex_buffer(...) |
| OP_GPU_CMD_BIND_IB | 77 | 82 | gpu.cmd_bind_index_buffer(...) |
| OP_GPU_CMD_DRAW_IDX | 78 | 83 | gpu.cmd_draw_indexed(...) |
| OP_GPU_SUBMIT_SYNC | 79 | 84 | gpu.submit_with_sync(...) |
| OP_GPU_ACQUIRE_IMG | 80 | 85 | gpu.acquire_next_image(sem) |
...
| OP_GPU_CMD_PUSH_CONST | 85 | 90 | gpu.cmd_push_constants(...) |
...
| OP_YIELD | 90 | 61 | Yield from generator [Collision: PUSH_CONST] |
| OP_CREATE_GENERATOR | 91 | 62 | Create generator [Collision: DISPATCH] |
| OP_GENERATOR_NEXT | 92 | 63 | Resume generator |
