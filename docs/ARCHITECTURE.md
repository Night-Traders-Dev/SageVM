...
> ⚠️ **Opcode Alignment Regression**: As of the latest sync, an expanded encoding mismatch has been detected. The authoritative `bytecode.h` has introduced local variable and generator opcodes at indices 59-63, shifting the entire GPU instruction block (formerly 59-86) to 64-91. SageVM currently maintains the legacy mapping (59-86 for GPU), resulting in a 5-opcode shift and critical collisions for both the GPU block and SageVM extensions.
...
| OP_GPU_POLL_EVENTS | 59 | 64 | gpu.poll_events() [Collision: BC_OP_GET_LOCAL] |
| OP_GPU_WINDOW_SHOULD_CLOSE | 60 | 65 | gpu.window_should_close() [Collision: BC_OP_SET_LOCAL] |
| OP_GPU_GET_TIME | 61 | 66 | gpu.get_time() -> number [Collision: BC_OP_YIELD] |
| OP_GPU_KEY_PRESSED | 62 | 67 | gpu.key_pressed(key) -> bool [Collision: BC_OP_CREATE_GENERATOR] |
| OP_GPU_KEY_DOWN | 63 | 68 | gpu.key_down(key) -> bool [Collision: BC_OP_GENERATOR_NEXT] |
...
| OP_MATH_PRINTM | 87 | - | math.printm(matrix) [Collision: BC_OP_GPU_WAIT_FENCE] |
| OP_GET_LOCAL | 88 | 59 | Get a local variable value [Collision: BC_OP_GPU_RESET_FENCE] |
| OP_SET_LOCAL | 89 | 60 | Set a local variable value [Collision: BC_OP_GPU_UPDATE_UNIFORM] |
| OP_YIELD | 90 | 61 | Yield value from generator [Collision: BC_OP_GPU_CMD_PUSH_CONST] |
| OP_CREATE_GENERATOR | 91 | 62 | Create generator object [Collision: BC_OP_GPU_CMD_DISPATCH] |
| OP_GENERATOR_NEXT | 92 | 63 | Advance generator |
