...
> ⚠️ **Opcode Alignment Regression**: As of the latest sync, a major encoding mismatch has been detected. The authoritative `bytecode.h` has introduced five new opcodes (`BC_OP_GET_LOCAL`, `BC_OP_SET_LOCAL`, `BC_OP_YIELD`, `BC_OP_CREATE_GENERATOR`, `BC_OP_GENERATOR_NEXT`) at indices 59-63, shifting the entire GPU instruction block (formerly 59-86) to 64-91. SageVM currently maintains the legacy mapping (59-86 for GPU), resulting in a 5-opcode shift and critical collisions for SageVM extensions (e.g., `OP_GET_LOCAL` at 88 vs. authoritative 59).
...
| OP_GPU_CMD_PUSH_CONST | 85 | 90 | gpu.cmd_push_constants(cmd, layout, stages, data) [Collision: YIELD] |
| OP_GPU_CMD_DISPATCH | 86 | 91 | gpu.cmd_dispatch(cmd, gx, gy, gz) [Collision: CREATE_GENERATOR] |
