...
- **Sandbox Hardening (Internal Properties & Method Definitions)**: In `safe_mode`, both SVM and SRVM interpreters block property, index, and method access (via `OP_CALL_METHOD`) for all identifiers starting with `__` (except `__arg`), preventing guest code from inspecting internal VM state or leaking host bridge objects. In addition, SVM's `OP_METHOD` and `OP_INHERIT` handlers enforce `safe_mode` restrictions against defining internal `__` methods, inheriting protected parent objects, or modifying protected class objects.
- **Builtin Dispatch Hardening (SRVM)**: SRVM `VMO_CALL` enforces explicit `safe_mode` and `ffi_enabled` checks for low-level `mem_*`, `ffi_*`, and `struct_*` builtin function dispatching to prevent unauthorized host memory or FFI operations under RISC-V mode.
