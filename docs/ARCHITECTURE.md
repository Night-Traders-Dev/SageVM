...
> ⚠️ **Opcode Alignment Regression**: As of the latest sync, a major encoding mismatch has been detected. The authoritative `bytecode.h` now defines a 5-opcode prefix (indices 59-63) for local variables and generators (`GET_LOCAL`, `SET_LOCAL`, `YIELD`, `CREATE_GENERATOR`, `GENERATOR_NEXT`), shifting the entire GPU instruction block (formerly 59-86) to 64-91. SageVM currently maintains the legacy mapping (59-86 for GPU), resulting in a 5-opcode shift across the Phase 16 instruction set and multiple collisions for SageVM extensions.
...
| OP_CREATE_GENERATOR | 91 | 62 | Define generator function [Collision: DISPATCH] |
