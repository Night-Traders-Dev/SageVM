...
- `class_inheritance_edge.sage`: Tests multi-level class inheritance (`OP_CLASS`, `OP_METHOD`, `OP_INHERIT`), method overriding, inherited method lookup, and child class instantiation.
- `exception_try_edge.sage`: Tests `try-catch` blocks (`OP_SETUP_TRY`, `OP_END_TRY`, `OP_RAISE`), clean execution, raising primitive values (`404`, `false`, `nil`), and re-raising errors in nested try blocks.
