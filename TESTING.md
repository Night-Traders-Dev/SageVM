...
- `class_inheritance_edge.sage`: Tests multi-level class inheritance (`OP_CLASS`, `OP_METHOD`, `OP_INHERIT`), method overriding, inherited method resolution, and missing method calls on instances.
- `exception_try_edge.sage`: Tests `try-catch` blocks (`OP_SETUP_TRY`, `OP_END_TRY`, `OP_RAISE`), clean try block completion, raising primitive values (`raise 404`, `raise false`, `raise nil`), structured array exception handling, and re-raising exceptions across nested blocks.
