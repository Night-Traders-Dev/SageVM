...
- `class_inheritance_edge.sage`: Tests multi-level class inheritance (`OP_CLASS`, `OP_METHOD`, `OP_INHERIT`), method overriding, inherited method resolution across ancestor classes, and error output when calling a missing method on an instance.
- `exception_try_edge.sage`: Tests `try-catch` blocks (`OP_SETUP_TRY`, `OP_END_TRY`, `OP_RAISE`) with clean execution without throwing, raising primitive values (`raise 404`, `raise false`, `raise nil`), and re-raising non-primitive array exceptions across nested `try-catch` blocks.
