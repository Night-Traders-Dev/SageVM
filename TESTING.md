...
- `class_inheritance_edge.sage`: Tests multi-level class inheritance (`OP_CLASS`, `OP_METHOD`, `OP_INHERIT`), method overriding, inherited method lookups/dispatch/resolution across ancestor classes, superclass value access, child instantiation, missing method/property calls on instances, and error output format.
- `exception_try_edge.sage`: Tests `try-catch` blocks (`OP_SETUP_TRY`, `OP_END_TRY`, `OP_RAISE`) with clean execution, raising primitive values (`raise 404`, `raise false`, `raise nil`), collection exceptions with length/index checks, nested unwinding, and re-raising array exceptions across nested blocks.
