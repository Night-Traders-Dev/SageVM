...
## 2026-07-25 - Sandbox Information Leak via Direct Builtin sys.getenv Calls in SVM
**Vulnerability:** The SVM sandbox restricts access to standard library modules (like `sys`) in `safe_mode` by removing them from `globals` and blacklisting them during `OP_IMPORT`. However, the direct call dispatcher allowed guest code to directly invoke `__builtin_sys_getenv` by name, bypassing the module-level restrictions and leaking sensitive host environment variables.
**Learning:** Restricting access to modules at namespace or import level is insufficient if underlying internal builtin functions can still be directly called by name from guest code. All sensitive builtins must explicitly validate sandbox/safe-mode flags inside their dispatch handlers.
**Prevention:** Always implement explicit security checks within the VM's builtin call dispatcher, independent of how the function was resolved or accessed by the guest program.

## 2026-07-27 - Direct Builtin Bypass & Modification Parity Hardening in SVM
**Vulnerability:** Direct calls to FFI builtins (`__builtin_ffi_open`, `__builtin_ffi_close`, `__builtin_ffi_call`) bypassed `--no-ffi` disablement checks inside the SVM dispatcher because they only checked `safe_mode`. Furthermore, `__builtin_push` and `__builtin_pop` used inconsistent/overly restrictive type checks against any dictionary under `safe_mode`, rather than delegating validation to `is_protected(obj)`.
**Learning:** Checking safety or module restrictions only at module resolution/import-time is a critical sandbox evasion vector. Any direct-callable internal builtin must duplicate environment validation controls. Additionally, write-protection lists must consistently query the `is_protected` security helper instead of making local type assumptions.
**Prevention:** Implement unified `self.ffi_enabled` and `self.is_protected` checks at the very entry points of container-mutation and FFI builtins.

## 2026-07-29 - Sandbox Information Leak via Global Dict Reflection in SVM
**Vulnerability:** The SVM interpreter's direct dictionary reflection builtins (`dict_has`, `dict_keys`, and `dict_values`) did not filter out internal-use properties starting with `__` (such as `__host_mod__`) in `safe_mode`. This allowed guest code to discover and leak internal VM metadata and underlying host bridge objects associated with global module configurations.
**Learning:** Hardening namespace property accesses via opcodes (`OP_GET_PROPERTY`, `OP_SET_PROPERTY`) is insufficient if standard reflection-like builtins still expose raw dictionary contents. Security checks and filters must be consistently applied across both the compiler's generated instruction sequence and the runtime's standard library builtin handlers.
**Prevention:** Mirror security filters and key-prefix blacklists in standard reflection builtins (`dict_has`, `dict_keys`, `dict_values`) when running in `safe_mode` across all execution backends.

## 2026-08-10 - Dynamic Execution Restriction Bypass via CLI Flag Disconnect and Unhardened Builtins
