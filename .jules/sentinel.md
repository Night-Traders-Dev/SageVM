...
## 2026-07-24 - Sandbox Parity Gap for push/pop in SVM Backend
**Vulnerability:** While the SRVM (RISC-V VM) backend correctly enforced `is_protected` checks for container mutation builtins `push`/`pop` under `safe_mode`, the SVM backend lacked these checks. This allowed a sandboxed guest executing in SVM to bypass sandbox restrictions and modify protected objects via high-level list/dictionary operations.
**Learning:** In multi-backend virtual machine environments, security validation implemented for high-level standard library builtins must be consistently mirrored across all dispatcher implementations. Backend parity is vital for complete sandbox integrity.
**Prevention:** Maintain strict parity in security controls across all independent VM dispatchers/executors. Ensure that security checks like `is_protected` are consistently applied across both SVM and SRVM standard library bridges.
