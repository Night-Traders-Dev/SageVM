...
## 2026-07-18 - Out-of-Bounds Stack Access in SVM Hot Loop
**Vulnerability:** Inlined dispatcher instructions (OP_GET_LOCAL, OP_SET_LOCAL, and OP_DUP) in SVM lacked bounds checking, bypassing the validation checks implemented in the non-inlined execute_op handler. Malformed bytecode could trigger host-level IndexError exceptions, causing Denial of Service (DoS) or interpreter crashes.
**Learning:** Performance optimization (inlining instructions into a hot loop) often introduces security regressions if the bounds checks from the original handlers are not carried over. When inlining stack mutation (like OP_SET_LOCAL stack-padding), care must be taken to maintain stack top integrity (via pop/push) so as not to corrupt VM state.
**Prevention:** Always ensure that optimization/inlining passes explicitly copy and enforce the underlying security check logic, and run tests with empty or minimal stack contexts to detect hidden regressions.
