...
## 2026-08-06 - Stack Peeking & Postponed Array Popping in VM Loop
**Learning:** In virtual machine interpreters, frequent stack modifications (`pop` operations) during arithmetic and comparison operations cause significant array resizing, elements shifting, and garbage collection overhead in the host environment. By instead directly reading values from the stable array using cached stack offsets (`stack_len-1` and `stack_len-2`), performing the operation, overwriting the resulting index, and deferring the mutating `pop` operation to the end of the handler, we avoid intermediate state changes. This yields a massive speedup on benchmarks (e.g. ~24.6% speedup on loop benchmarks) and drops host system/GC overhead by up to ~83%. Furthermore, always verify that security/bounds checks are positioned before cached collection lookups to prevent malformed bytecodes from raising unhandled index exceptions.
**Action:** Use stack-peeking (relative indexing) and postponed popping for multi-operand operations to minimize array resizing and host-level mutation overhead, and always order bounds-checks before cache retrievals.
