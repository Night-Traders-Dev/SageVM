...
## 2026-08-10 - Numerical Operand Fast-path Checks bypassing type() Builtin
**Learning:** Calling the standard `type()` builtin function inside the SVM interpreter's high-frequency arithmetic and comparison opcodes (OP_ADD, OP_SUB, OP_MUL, OP_DIV, OP_MOD, OP_LESS, OP_LESS_EQUAL, OP_GREATER, OP_GREATER_EQUAL) inside `MetalVM.run` induces expensive heap string allocations (such as C `strdup` calls) in the emitted compiler codebase. Introducing a non-allocating fast-path check `a != nil and b != nil and tonumber(a) == a and tonumber(b) == b` directly checks if both operands are numbers without allocating memory or calling `type()`. Bypassing this for common numerical operations reduces loop-heavy benchmark execution times by ~27.5% (from ~10.5s to ~7.6s) and slashes system/GC overhead by over 98.5% (from ~1.9s to ~0.02s).
**Action:** Always bypass expensive type checking functions inside interpreter hot loops by using fast-path checks with `tonumber(x) == x` when type equivalence matches.
